...
def _normalise_for_containment(text: str) -> str:
    """
    Reduce text to lowercase alphanumerics for anchor matching.

    Whitespace is removed entirely, not just collapsed. Word stores a comment's
    anchor as a sequence of runs, and formatting boundaries split words
    mid-token: real anchors in the GLATT document include "C IP Unit",
    "A TEX certified" and "sh ould". Comparing on letters alone lets those match
    the requirement text they actually came from.
    """
...
def _pair_comments_to_requirements(pairs: List[Dict[str, Any]],
    ...
    """
    Attach each DOCX comment to the requirement it is actually anchored to.

    A DOCX comment records the exact run of text it covers. That anchor is
    structural fact, so it is used first and semantic similarity is only a
    fallback for when extraction reworded the text.

    Three cases, in order:

    1. The anchor sits inside a requirement - the normal "comment on one
       requirement" case.
    2. A requirement sits inside the anchor - the anchor covers a table or a
       whole section, so every requirement within it gets the comment.
    3. Neither contains the other - fall back to the single best semantic
       match above COMMENT_MATCH_THRESHOLD.

    Containment requires an anchor of at least MIN_ANCHOR_CHARS so that short
    spans ("Yes", "Required") cannot latch onto unrelated requirements.

    Returns the number of comments attached.
    """
    try:
        from config import COMMENT_MATCH_THRESHOLD as THRESHOLD
    except ImportError:
        THRESHOLD = COMMENT_MATCH_THRESHOLD
    ...
    norm_reqs = []
    for pair in pairs:
        text = (pair['requirement'].get('text') or '').strip()
        norm_reqs.append((pair, text, _normalise_for_containment(text)))

    attached = 0

    def attach(pair, comments, anchor, score, how):
        nonlocal attached
        ...
        for comment in comments:
            ...
            pair['comments'].append({
                ...
                'match_method': how,
                'matched_to': anchor[:100],
            })
            existing.add(text)
            attached += 1

    for anchor, comments in structured.items():
        anchor = (anchor or '').strip()
        norm_anchor = _normalise_for_containment(anchor)
        if not norm_anchor:
            continue

        # 1 + 2: structural containment, either direction.
        matched_any = False
        if len(norm_anchor) >= MIN_ANCHOR_CHARS:
            for pair, req_text, norm_req in norm_reqs:
                if not norm_req:
                    continue
                if norm_anchor in norm_req:
                    attach(pair, comments, anchor, 1.0, 'anchor_in_requirement')
                    matched_any = True
                elif norm_req in norm_anchor:
                    # Anchor spans a table or section containing this
                    # requirement; every requirement inside it is commented.
                    attach(pair, comments, anchor, 1.0, 'requirement_in_anchor')
                    matched_any = True

        if matched_any:
            continue

        # 3: semantic fallback - one best requirement only.
        best_pair = None
        best_score = 0.0
        for pair, req_text, _ in norm_reqs:
            if not req_text:
                continue
            if matcher:
                score = matcher.calculate_semantic_similarity(req_text, anchor)
            else:
        ...
        if best_pair is not None and best_score >= THRESHOLD:
            attach(best_pair, comments, anchor, best_score, 'semantic')
        else:
            logger.info("No requirement for comment anchored to %r (best %.3f)",
                        anchor[:60], best_score)
