...
try:
    ...
    GEMINI_AVAILABLE = True and GEMINI_API_KEY != "your_gemini_api_key_here"
except ImportError:
...
def extract_with_gemini_enhancement(uploaded_file, api_key: str = None) -> List[Dict[str, Any]]:
    """
    Enhanced extraction pipeline using Gemini Pro for intelligent requirement detection.
    
    This function combines traditional extraction with Gemini's advanced text understanding
    to provide higher quality requirement extraction and categorization.
    ...
    Returns:
        List of enhanced page dictionaries with Gemini-processed requirements
    """
    # First, perform standard extraction
    standard_pages = extract_structured_content(uploaded_file)
    
    if not GEMINI_AVAILABLE or not api_key:
        print("Gemini not available or no API key provided, falling back to standard extraction")
    ...
    try:
        ...
        for page in standard_pages:
            ...
            print(f"Processing page {page.get('page_number', 'unknown')} with Gemini...")
            
            # Extract requirements using Gemini
            gemini_requirements = gemini_processor.extract_requirements_from_text(
                content, 
                document_type="technical_specification"
            )
            
            # Create enhanced page structure
            enhanced_page = page.copy()
            enhanced_page["gemini_requirements"] = gemini_requirements
            enhanced_page["gemini_processed"] = True
            enhanced_page["requirement_count"] = len(gemini_requirements)
            
            # Add individual requirement pages for better vectorization
            if gemini_requirements:
                for i, req in enumerate(gemini_requirements):
                    req_page = {
                        "page_number": f"{page.get('page_number', 1)}-GEM-{i+1}",
                        "content": req["text"],
                        "content_type": f"{page.get('content_type', 'unknown')}-gemini",
                        "section": req.get("section", page.get("section", "")),
                        "category": req.get("category", "technical"),
                        "priority": req.get("priority", "standard"),
                        "confidence": req.get("confidence", 0.8),
                        "source": "gemini_pro",
                        "original_page": page.get("page_number", 1),
                        "tables": [],
                        "comments": page.get("comments", []),
                        "gemini_processed": True
                    }
                    enhanced_pages.append(req_page)
            
            # Also keep the original page with enhancement metadata
            enhanced_pages.append(enhanced_page)
        
        print(f"Gemini processing complete. Enhanced {len(standard_pages)} pages into {len(enhanced_pages)} items")
...
def batch_enhance_requirements_with_gemini(requirements: List[str], api_key: str = None) -> List[Dict[str, Any]]:
    """
    Enhance a batch of requirements using Gemini Pro for better categorization and quality.
    ...
    """
    if not GEMINI_AVAILABLE or not api_key or not requirements:
        return [{"text": req, "category": "technical", "confidence": 0.5} for req in requirements]
    
    try:
        gemini_processor = GeminiProcessor(api_key)
        enhanced_requirements = gemini_processor.categorize_requirements(requirements)
        
        print(f"Enhanced {len(requirements)} requirements with Gemini categorization")
        return enhanced_requirements
        
    except Exception as e:
        print(f"Error enhancing requirements with Gemini: {e}")
        return [{"text": req, "category": "technical", "confidence": 0.5} for req in requirements]


def preprocess_document_with_gemini(file_content: str, api_key: str = None) -> Dict[str, Any]:
    """
    Preprocess entire document content with Gemini for structure understanding.
    ...
    Returns:
        Dictionary with document analysis and preprocessing results
    """
    if not GEMINI_AVAILABLE or not api_key:
        return {"processed": False, "content": file_content}
    
    try:
        gemini_processor = GeminiProcessor(api_key)
        
        # Analyze document structure and extract requirements
        requirements = gemini_processor.extract_requirements_from_text(
            file_content[:4000],  # Limit for API efficiency
            document_type="technical_specification"
        )
        ...
        preprocessing_result = {
            ...
            "gemini_requirements": requirements,
            "document_type": "technical_specification",
            "preprocessing_timestamp": str(datetime.now())
        }
